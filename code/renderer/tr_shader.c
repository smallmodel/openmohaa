...
static qboolean ParseStage( shaderStage_t *stage, char **text )
{
	...
	while ( 1 )
	{
		...
		else if ( !Q_stricmp( token, "map" ) )
		{
			...
			{
				stage->bundle[bundleNum].image[0] = R_FindImageFile( token, !shader.noMipMaps, !shader.noPicMip, GL_REPEAT );
		...
		else if ( !Q_stricmp( token, "clampmap" ) || !Q_stricmp( token, "clampmapy" ) )	// IneQuation: I know it's silly, but it fixes the shingle texture on Omaha
		{
			...
			stage->bundle[bundleNum].image[0] = R_FindImageFile( token, !shader.noMipMaps, !shader.noPicMip, GL_CLAMP_TO_EDGE );
			if ( !stage->bundle[bundleNum].image[0] )
			{
				ri.Printf( PRINT_WARNING, "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n", token, shader.name );
				return qfalse;
		...
		else if ( !Q_stricmp( token, "animMap" ) )
		{
			...
			while ( 1 ) {
				...
				if ( num < MAX_IMAGE_ANIMATIONS ) {
					stage->bundle[bundleNum].image[num] = R_FindImageFile( token, !shader.noMipMaps, !shader.noPicMip, GL_REPEAT );
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
// su44: in order to fix black-rects-in-ui bug (which seem to appear only on NVidia cards),
// I've replaced GL_CLAMP with GL_CLAMP_TO_EDGE in the entire renderer
