...
void R_LoadTerrain(lump_t *ter) {
	...
	for (i = 0; i < numPatches; i++) {
		...
		out[i].texCoords[1] = ((float)in[i].lmCoords[0] + 0.5) / LIGHTMAP_SIZE;
		out[i].texCoords[3] = ((float)in[i].lmCoords[1] + 0.5) / LIGHTMAP_SIZE;
		out[i].texCoords[5] = ((float)in[i].lmCoords[0] + 16.5) / LIGHTMAP_SIZE;
		out[i].texCoords[7] = ((float)in[i].lmCoords[1] + 16.5) / LIGHTMAP_SIZE;
	...
	outI = ri.Hunk_Alloc(numIndices * sizeof(*outI), h_low);

	s_worldData.terrainIndices = outI;
	s_worldData.numTerrainIndices = numIndices;
	...
	for(i = 0; i < s_worldData.numStaticModels; i++) {
		...
		*ptr2 = 0;
		...
		if(s_worldData.staticModels[i].model != 0) {
			...
			ri.TIKI_AppendFrameBoundsAndRadius(tiki,0,0,&s_worldData.staticModels[i].radius,s_worldData.staticModels[i].bounds);
...
void RE_LoadWorldMap( const char *name ) {
	...
	R_LoadFogs( /*&header->lumps[LUMP_FOGS]*/ &emptyLump, &header->lumps[LUMP_BRUSHES], &header->lumps[LUMP_BRUSHSIDES] );
