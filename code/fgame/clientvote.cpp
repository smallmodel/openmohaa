...
bool VoteUpload::ClientThink()
{
    ...
    if (lastTime != -1 && gi.Milliseconds() < lastTime + 50) {
        return false;
    }

    lastTime = gi.Milliseconds();
    ...
    gi.Printf(
        "sent: %d - ping: %d - commandTime: %d - time: %d\n",
        level.framenum,
        game.clients[clientNum].ps.ping,
        game.clients[clientNum].ps.commandTime,
        current_ucmd->serverTime
    );
