...
// su44: added by me, used for testing pathfinding on MoHAA maps
int AINode_GoToEntity(bot_state_t *bs) {
	...
	if (BotIsObserver(bs)) {
		AIEnter_Observer(bs, "goToEntity: observer");
		return qfalse;
	}
	//if in the intermission
	if (BotIntermission(bs)) {
		AIEnter_Intermission(bs, "goToEntity: intermission");
		return qfalse;
	}
	//respawn if dead
	if (BotIsDead(bs)) {
		AIEnter_Respawn(bs, "goToEntity: bot dead");
		return qfalse;
	}
	//if no enemy
	if (bs->enemy < 0) {
		AIEnter_Seek_LTG(bs, "goToEntity: no enemy");
		return qfalse;
	}
	bs->tfl = TFL_DEFAULT;
	...
	// su44: update goal entity areanum and origin all the time
	VectorCopy(g_entities[0].s.origin,bs->lastenemyorigin);
	bs->lastenemyareanum = BotPointAreaNum(bs->lastenemyorigin);
...
int AINode_Battle_Fight(bot_state_t *bs) {
	...
	// su44: FIXME -> is there a PERS_HITS in MoH ?
